...
$repoOwner = "SagerNet"
$repoName = "sing-box"
$tempZipFile = $null # 后续动态赋值
$targetExePath = '..\release\sing-box.exe'
$tempUnzipDir = ".\.temp_xray_unzip" # 临时解压目录（仅存放解压的压缩包内容）
$cpFileName = "sing-box.exe" # 要拷贝的文件
...
try {
    ...
    # Step 2: 解析最新版本号和下载链接
    $latestVersionWithV = $latestRelease.tag_name # 原始版本号（如v1.12.21）
    $latestVersion = $latestVersionWithV.TrimStart('v') # 去除v前缀，得到1.12.21
    Write-Host "Found latest version: $latestVersion (original: $latestVersionWithV)"

    # 动态生成assetName（此时$latestVersion已有值，且无v前缀）
    $assetName = "sing-box-$latestVersion-windows-amd64.zip"
    $tempZipFile = ".\$assetName" # 同步更新临时zip文件名
    ...
    $tempExePath = Join-Path $tempUnzipDir\sing-box-$latestVersion-windows-amd64 $cpFileName    
