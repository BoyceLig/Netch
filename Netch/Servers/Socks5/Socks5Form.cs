...
public class Socks5Form : ServerForm
{
    public Socks5Form(Socks5Server? server = default)
    {
        ...
        CreateComboBox("Version", "Version", SOCKSGlobal.Versions, s => server.Version = s, server.Version);
        (_remoteHostnameLabel, _remoteHostnameTextBox) = CreateTextBox("RemoteHostname", "Remote Address", s => true, s => server.RemoteHostname = s, server.RemoteHostname.ValueOrDefault());
